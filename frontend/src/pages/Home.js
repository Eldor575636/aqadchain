...
function CarSVG({ type }) {
  if (type === 'suv') return (
    <svg viewBox="0 0 200 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      <rect x="10" y="30" width="180" height="32" rx="4" fill="rgba(13,110,99,0.6)" stroke="rgba(13,110,99,0.9)" strokeWidth="1.5"/>
      <path d="M30 30 L50 8 L150 8 L170 30" fill="rgba(13,110,99,0.5)" stroke="rgba(13,110,99,0.9)" strokeWidth="1.5"/>
      <rect x="55" y="12" width="88" height="16" rx="2" fill="rgba(255,255,255,0.05)" stroke="rgba(255,255,255,0.15)" strokeWidth="1"/>
      <circle cx="45" cy="64" r="12" fill="rgba(10,82,72,0.8)" stroke="rgba(13,110,99,1)" strokeWidth="2"/>
      <circle cx="45" cy="64" r="5" fill="rgba(13,110,99,0.4)"/>
      <circle cx="155" cy="64" r="12" fill="rgba(10,82,72,0.8)" stroke="rgba(13,110,99,1)" strokeWidth="2"/>
      <circle cx="155" cy="64" r="5" fill="rgba(13,110,99,0.4)"/>
      <rect x="165" y="34" width="22" height="8" rx="2" fill="rgba(201,168,76,0.6)"/>
      <rect x="10" y="34" width="12" height="8" rx="2" fill="rgba(255,100,100,0.5)"/>
    </svg>
  );
  if (type === 'truck') return (
    <svg viewBox="0 0 220 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      <rect x="10" y="28" width="110" height="36" rx="3" fill="rgba(13,110,99,0.6)" stroke="rgba(13,110,99,0.9)" strokeWidth="1.5"/>
      <path d="M25 28 L40 8 L110 8 L110 28" fill="rgba(13,110,99,0.5)" stroke="rgba(13,110,99,0.9)" strokeWidth="1.5"/>
      <rect x="110" y="34" width="100" height="30" rx="2" fill="rgba(13,110,99,0.35)" stroke="rgba(13,110,99,0.7)" strokeWidth="1.5"/>
      <rect x="42" y="12" width="62" height="14" rx="2" fill="rgba(255,255,255,0.05)" stroke="rgba(255,255,255,0.12)" strokeWidth="1"/>
      <circle cx="40" cy="65" r="12" fill="rgba(10,82,72,0.8)" stroke="rgba(13,110,99,1)" strokeWidth="2"/>
      <circle cx="40" cy="65" r="5" fill="rgba(13,110,99,0.4)"/>
      <circle cx="155" cy="65" r="12" fill="rgba(10,82,72,0.8)" stroke="rgba(13,110,99,1)" strokeWidth="2"/>
      <circle cx="155" cy="65" r="5" fill="rgba(13,110,99,0.4)"/>
      <circle cx="185" cy="65" r="12" fill="rgba(10,82,72,0.8)" stroke="rgba(13,110,99,1)" strokeWidth="2"/>
      <circle cx="185" cy="65" r="5" fill="rgba(13,110,99,0.4)"/>
      <rect x="195" y="32" width="18" height="8" rx="2" fill="rgba(201,168,76,0.6)"/>
      <rect x="10" y="32" width="10" height="8" rx="2" fill="rgba(255,100,100,0.5)"/>
    </svg>
  );
  return (
    <svg viewBox="0 0 200 70" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      <rect x="10" y="32" width="180" height="26" rx="5" fill="rgba(13,110,99,0.6)" stroke="rgba(13,110,99,0.9)" strokeWidth="1.5"/>
      <path d="M35 32 L60 10 L145 10 L168 32" fill="rgba(13,110,99,0.5)" stroke="rgba(13,110,99,0.9)" strokeWidth="1.5"/>
      <rect x="65" y="14" width="72" height="14" rx="2" fill="rgba(255,255,255,0.05)" stroke="rgba(255,255,255,0.12)" strokeWidth="1"/>
      <circle cx="48" cy="60" r="10" fill="rgba(10,82,72,0.8)" stroke="rgba(13,110,99,1)" strokeWidth="2"/>
      <circle cx="48" cy="60" r="4" fill="rgba(13,110,99,0.4)"/>
      <circle cx="152" cy="60" r="10" fill="rgba(10,82,72,0.8)" stroke="rgba(13,110,99,1)" strokeWidth="2"/>
      <circle cx="152" cy="60" r="4" fill="rgba(13,110,99,0.4)"/>
      <rect x="168" y="36" width="20" height="7" rx="2" fill="rgba(201,168,76,0.6)"/>
      <rect x="10" y="36" width="10" height="7" rx="2" fill="rgba(255,100,100,0.5)"/>
    </svg>
  );
}
...
const carSlides = [
  {
    type: 'sedan',
    label: 'Sedan',
    slogan: 'Halal financing for everyday cars.',
    sub: 'Murabaha & Musawama — cost-disclosed or negotiated price.',
    color: '#0D6E63',
  },
  {
    type: 'suv',
    label: 'Luxury SUV',
    slogan: 'Big purchases deserve Shariah integrity.',
    sub: 'No Riba. No hidden fees. Just a clean halal agreement.',
    color: '#C9A84C',
  },
  {
    type: 'truck',
    label: 'Work Truck',
    slogan: 'Ijarah leases for fleet & commercial.',
    sub: 'Lease any vehicle the halal way — IMIT or operating lease.',
    color: '#3B82F6',
  },
];

function AnimatedCarHero({ onSignUp }) {
  const [idx, setIdx] = useState(0);
  const [phase, setPhase] = useState('in'); // 'in' | 'hold' | 'out'

  useEffect(() => {
    let t;
    if (phase === 'in') t = setTimeout(() => setPhase('hold'), 900);
    else if (phase === 'hold') t = setTimeout(() => setPhase('out'), 3000);
    else {
      t = setTimeout(() => {
        setIdx(i => (i + 1) % carSlides.length);
        setPhase('in');
      }, 700);
    }
  ...
  const slide = carSlides[idx];

  const carX = phase === 'in' ? ['120%', '0%'] : phase === 'out' ? ['0%', '-120%'] : ['0%', '0%'];
  const carDuration = phase === 'hold' ? 0 : phase === 'in' ? 0.85 : 0.65;

  return (
    <div className="w-full max-w-xl mx-auto relative select-none">
      {/* Road */}
      ...
      <div className="relative h-36 overflow-hidden">
        <motion.div
          key={`car-${idx}-${phase}`}
          animate={{ x: carX }}
          transition={{ duration: carDuration, ease: phase === 'in' ? [0.25,0.46,0.45,0.94] : [0.55,0,1,0.45] }}
          className="absolute inset-0 flex items-end pb-2 px-4"
        >
          <div className="w-full" style={{ filter: `drop-shadow(0 0 24px ${slide.color}88)` }}>
            <CarSVG type={slide.type} />
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          style={{ background: slide.color + '55' }}
        />
      </div>

      {/* Dashed road line */}
      <div className="flex gap-3 justify-center mt-1 mb-6 opacity-15">
        {Array.from({ length: 10 }).map((_, i) => (
          <motion.div
            key={i}
            animate={{ x: phase === 'hold' ? [0, -8, 0] : 0 }}
            transition={{ duration: 0.4, delay: i * 0.04, repeat: phase === 'hold' ? Infinity : 0 }}
            className="w-8 h-0.5 rounded-full bg-white"
          />
        ))}
      </div>
    ...
    <div className="flex flex-col min-h-screen" style={{ background: '#080C0F', color: '#fff' }}>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-4 pt-20 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-5xl mx-auto w-full">
          ...
          >
            <AnimatedCarHero onSignUp={signUp} />
          </motion.div>
        </div>
